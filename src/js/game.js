...
export default class Game {
    ...
    setLCBottle(quantity) {
        ...
        if (quantity <= 12) {
            this.bottleW = WIDTH / 8;
            this.ballW = WIDTH / 11.6;
        }
        if (quantity > 15) {
            this.bottleW = WIDTH / 10;
            this.ballW = WIDTH / 13.6;
        }
        ...
        this.bottleBase = {
            ...
            startX: startX,
            startY: indexRow.row == 1 ? [HEIGHT / 3] : [HEIGHT / 4.5, HEIGHT / 2]
        };
        ...
        var heightBall = ball.getBounds().height * 0.975;
    ...
    setMap(indexRow) {
        ...
        // if (this.player.level == 1 || this.player.level == 2) this.renderTutorial()
    }
    getBall(color) {
        switch (color) {
            case 1:
    ...
    upBallChoose() {
        ...
        // console.log(this.listBottle);s
        // createjs.Tween.get(ball)
        //     .to({ y: y }, TIMEMOVEUP, createjs.Ease.linear);
    }
    downBallChoose() {
        var indexChoose = this.listBottleA[this.listBottleA.length - 1].index;
        ...
        // createjs.Tween.get(ball)
        //     .to({ y: y }, TIMEMOVEDOWN, createjs.Ease.sineIn)
        //     .call(() => {
        //         this.leapBall(ball)
        //         var sound_Pour = createjs.Sound.play("ball_fall_1");
        //         sound_Pour.volume = 1;
        //     })
    }
    leapBall(ball) {
    ...
    moveBallChoose() {
        // if (tutorialRun) this.clearTutorial();
        var complete = 0;
    ...
    drawWin() {
        ...
        btn_next.scale.set((this.app.screen.width / 3.2) / btn_next.width, (this.app.screen.width / 3.2) / btn_next.width);
        btn_next.position.set(-btn_next.width, this.app.screen.height * 0.56);
        this.containerWin.addChild(btn_next)
        btn_next.interactive = true;

        var tl = gsap.timeline();
        tl.delay(1)
        tl.to(btn_next, { duration: 0.6, x: (this.app.screen.width - btn_next.width) / 2, ease: "back.inOut" })
    ...
    setFireworkBelow() {
        ...
        const scale = Math.min((this.app.screen.width) / win.width, (this.app.screen.height) / win.height,);
        win.scale.set(scale, scale);
        win.position.set(0, -win.height * 0.1);
    ...
    drawSetting() {
        ...
        var scale_bg_setting = (WIDTH * 2 / 3) / bg_setting.width
        bg_setting.scale.set(scale_bg_setting, scale_bg_setting);
        bg_setting.position.set((WIDTH - bg_setting.width) / 2, bg_setting.height / 2);
